...
const SinglePost = () => {
	...
	useEffect(() => {
		const getPost = async () => {
			try {
				const res = await axios.get(`http://api.thefzz.com:3000/posts/${path}`)
				setPost(res.data)
	...
	const handleDelete = async () => {
		try {
			...
			await axios.delete(`http://api.thefzz.com:3000/posts/${path}`,config)
		...
		<div className="singlePost">
			...
			{post ? (
				<div className="singlePost__wrapper" ref={elementRef}>
					{post.imageUrl && (
						<img src={post.imageUrl} alt="" className="singlePost__img" crossOrigin="true" />
					)}
